...
type Row = {
  ...
  item: { item_number: number; name: string; sku: string; pieces_per_bag: number } | null;
  created_at: string;
};
...
export default function DispatchHistoryPage() {
  ...
  async function load() {
    ...
    let q = supabase
      .from("dispatches")
      .select("id,date,challan_no,qty_pieces,qty_bags,entered_qty_type,created_at,customer:customers(name),item:items(item_number,name,sku,pieces_per_bag)")
  ...
  return (
    <div className="space-y-4">
      <SectionHeader
        ...
        subtitle="Full outgoing entries. Filter by date range or search by customer/item/SKU/challan."
        right={
